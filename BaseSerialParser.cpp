...
{
	#ifdef DEBUG_COMM
	SerialUSB.println("received a msg");
	#endif
	if(t_msg->getType() == msg_type::SERIALDATA)
	{
		#ifdef DEBUG_COMM
		SerialUSB.println("received serialdata msg");
		#endif
		SerialDataMsg new_msg = *((SerialDataMsg*) t_msg);
		// for (int i = 0; i < new_msg.len; i++)
		// {
		// 	SerialUSB.write(new_msg.data[i]);
		// }
		for (int i = 0; i < new_msg.len; i++)
		{
			#ifdef DEBUG_COMM
			SerialUSB.println("parsing msg " + String(i) + " out of " + String(new_msg.len));
			#endif
			if (this->state_machine(new_msg.data[i]))
			{
				...
				if(t_type == msg_type::THREEAXISSENSORMSG)
				{
